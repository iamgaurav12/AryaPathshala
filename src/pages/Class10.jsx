...
const Class10 = () => {
  ...
  const calculateCompletionRate = (chapter) => {
    ...
    const availableResources = resources.filter(link => link && link.trim() !== '').length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {chapters.length > 0 && (
          <div className={`mb-6 p-4 rounded-lg border-l-4 border-orange-500 ${
            darkMode ? 'bg-orange-900/20 border-orange-400' : 'bg-orange-50'
          }`}>
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className={`text-sm mt-1 ${darkMode ? 'text-orange-300' : 'text-orange-700'}`}>
                  This is your final year! Focus on understanding concepts thoroughly and practice regularly. 
                  All chapters are aligned with the latest CBSE curriculum.
                </p>
              </div>
            </div>
          </div>
        ...
        {chapters.length > 0 && (
          <div className={`mt-8 p-6 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
            <h3 className="text-lg font-semibold mb-4">Board Exam Preparation Progress</h3>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-red-500">
                  {Math.round(chapters.reduce((acc, c) => acc + calculateCompletionRate(c), 0) / chapters.length)}%
                </div>
                <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Avg Completion</div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {subjects.slice(1).map(subject => {
                  ...
                  const avgCompletion = subjectChapters.length > 0 
                    ? Math.round(subjectChapters.reduce((acc, c) => acc + calculateCompletionRate(c), 0) / subjectChapters.length)
                    : 0;
                  
                  return (
                    <div key={subject} className={`p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-medium">{subject}</span>
                        <span className="text-sm text-gray-500">{avgCompletion}%</span>
                      </div>
